...
You are allowed to:
 - Create as many games with ORK Framework as you want.
 - Sell (or distribute for free) games created with ORK Framework, as long as they don't contain any plain code of ORK Framework (i.e. uncompiled code, only distribute built games).

You are not allowed to:
 - Redistribute for free, share or sell ORK Framework. You can only distribute or sell it in form of a built game.
...
There are 2 types of access to ORK Framework: Limited and Full.
...
- Code access to gameplay related code (editor and core functionality are closed source).


If you own 'ORK Okashi RPG Kit' (aka ORK 1) and the 'Menu/Shop Extension', you'll receive a free upgrade to ORK Framework.
Please send your order/invoice numbers to contact@orkframework.com.
...
- ORKFramework.unitypackage
3 DLLs containing ORKFramework.
Gizmo icons in 32x32 pixels.

- gameplay_source_code.zip ('Full Licence' only)
...
- fix: Equipment Viewers: Using the 'Add Combatant' component with 'Equipment Viewers' could lead to not displaying equipment if the combatant was initialized after the equipment viewer.
...
Version 2.0.0
- new: Editor: Game Variables: A new editor section allows adding and searching for game variables. All editor fields to define variables allow selecting variables from a list (press the button next to the variable field). The new section can be found in 'Editor > Game Variables'.
- new: Editor: Navigation history buttons available in the sub-section box. You can navigate back and forward in the visited sections, sub-sections and data indexes. Use 'Home' and 'End' as navigation hot-keys.
- new: Editor: Saving the project - display of the changing parts changed. Options available to update events, prefabs and scenes.
- new: GUI Layers: GUI layers are used to organize the order of GUI box displays and to allow filtering GUI boxes in lists. Can be created in 'Menus > GUI Layers'.
- new: Combatants: 'Conditional Prefabs' settings available. Change a combatant's prefab based on status and variable conditions.
- new: Event System: 'Child Object' setting available in all object selections. You can now use a child object of an actor, waypoint or prefab in all event steps that use objects. This will reset the previous child object settings in some steps.
- new: Event System: Movement Steps: 'Check Transform' step available. Check an object's position, rotation or scale - each axis (X, Y, Z) can be check individually.
- new: Event System: Movement Steps: 'Block Move AI' step available. Block or unblock the move AI of all combatants.
- new: Game Events: Event Settings: 'Block Move AI' setting available. Blocks the move AI of all combatants while the event performs.
- new: Game Events: Event Settings: 'Block Actor Move AI' setting available. Blocks the move AI of all actors of the event (if they have a combatant) while the event performs.
- new: Inventory: Item Collection: 'Show Dialogue' setting available. The collection dialogue for item collectors is now optional - if disabled, the item will be collected immediately. By default enabled.
- new: Shops: 'Music Settings' available. A shop can play a different music clip while opened. The currently playing clip will resume playback after the shop has been closed.
- new: Menu Screens: 'Music Settings' available. A menu screen can play a different music clip while opened. The currently playing clip will resume playback after the menu screen has been closed.
- new: HUDs: Combatant HUD: 'Set Size' settings available for status value, attack/defence attribute and defence attribute ID HUD elements. When listing data, you can set optionally the size of the individual items.
- new: HUDs: Tooltip HUD: An item collector's item is displayed by tooltip HUDs. Only for 'Single' or 'Random' collectors.
- new: HUDs: Combatant HUDs: 'Defence Attribute ID' has additional text codes to display information about the attribute.
- new: Control Maps: 'Type' setting available in control map keys. 'Action' uses a battle action, 'Shortcut' uses a shortcut slot, 'Auto Attack' enables/disables the auto attack. This will reset your 'Use Shortcut' setting in all control maps.
- new: Control Maps: 'Force Index' and 'Reset Index' settings available for 'Attack' action keys. Optionally force a base attack, even if it's not available due to the attack index. Optionally reset the attack index after using an attack.
- new: Battle Settings: Battle Advantage: 'First Phase' settings available. The group in advantage can perform a defined number of phases before other groups in 'Phase' type battles.
- new: Battles: 'Defeat on Player Death' setting available. Optionally end the battle when the player is dead, instead of the whole battle group. By default disabled.
- new: Battle Menus: Title settings available. Optionally show a title for battle menus.
- new: Status Values: 'Level Difference Factor' settings available for 'Experience' type status values. The received experience can optionally depend on the base/class level difference between defeated combatant and the combatant receiving experience.
- new: Weapons, Armors: 'Battle Sounds' settings available. Equipped weapons and armors can override the combatant's battle sounds. The selected sound types will be overridden.
- new: Equipment Viewer: 'Check Object Variables' setting available. Displaying the equipment can now also depend on object variables.
- change: Combatants: The prefab settings are now bundled in their own foldout.
- change: Battle Texts: The experience text is now displayed for each combatant. New text codes for combatant information available.
- change: Editor Settings: 'Show Save Changes' setting removed. The changes will now be displayed every time when saving.
- change: Move AI: Combatants will try hunting their last targets after finishing a battle action. The target is hunted if the 'Target Lost Interval' is used or the target can be detected by the move detection conditions.
- change: Move AI: If a battle system type doesn't allow using move AI, it's now blocked from the start (i.e. before start events) until the end (i.e. after end events).
- change: GUI Boxes: 'Above Screen Fader' setting has been removed. Use the new GUI layers to display GUI boxes above or below the screen fader using the 'Screen Fader' option in the GUI layer.
- fix: Editor: Closing Unity without closing the ORK Framework editor first wont result in errors when opening Unity again.
- fix: Abilities: Base and counter attacks now use the ability's combo and reuse settings.
- fix: Text Editor: The data selection popup field is now displayed correctly.
- fix: Battle System: Auto attacks are now working in real time battles.
- fix: Level Up Bonuses: 'Full Recovery' of all 'Consumable' type status values will now happen after the level up and status value changes, making sure the new values are used.
- fix: Equipment Parts: Initializing a combatant's equipment parts is now based on the correct class.
- fix: Editor: The ORK Framework editor now also works while having 'Web Player' selected as build target.
- fix: Menu Screens: Descriptions are now displayed correctly.
- fix: Interactions: Scene changers, item collectors and other interactions wont start multiple times when using 'Key Press' start type and hitting the key multiple times.
- fix: HUDs: 'Normal' type status value with 'Combined Value' enabled now display the correct value.
